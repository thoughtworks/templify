...
public class PlainTextHandlerTest {
    ...
    public void test_find_simple_types_must_not_return_values(String query)
        ...
        System.out.println("query: " + query + " == " + this.plainTextHandler.find(teste_txt, query));
        assertThat(this.plainTextHandler.find(teste_txt, query)).isEmpty();
    }

    @ParameterizedTest
    @CsvSource({"hobbies","David","football"})
    public void test_find_something_existing_must_return_result(String query) throws FileHandlerException {
    ...
    public void test_replace_with_map() throws Exception {
        ...
        assertThat(findUpdatedContent).isNotNull().isNotEmpty().containsKey(newName).containsValue("2");
    }
    ...
    public void test_replace_with_maps() throws Exception {
        ...
        String conditionStartPlaceholder = "#backstage-template-condition-infra-elk-condition-start";
        ...
        Map<String, String> previousPlaceholder1 = this.plainTextHandler.find(fileForTest.toAbsolutePath().toString(), conditionStartPlaceholder);
        Map<String, String> previousPlaceholder2 = this.plainTextHandler.find(fileForTest.toAbsolutePath().toString(), conditionEndPlaceholder);
        Map<String, String> updatedPlaceholder1 = this.plainTextHandler.find(fileForTest.toAbsolutePath().toString(), conditionStartValue);
        Map<String, String> updatedPlaceholder2 = this.plainTextHandler.find(fileForTest.toAbsolutePath().toString(), conditionEndValue);

        assertThat(previousPlaceholder1).isEmpty();
        assertThat(previousPlaceholder2).isEmpty();
        assertThat(updatedPlaceholder1).isNotNull().isNotEmpty().containsKey(conditionStartValue).containsValue("1");
        assertThat(updatedPlaceholder2).isNotNull().isNotEmpty().containsKey(conditionEndValue).containsValue("1");
